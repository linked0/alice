#!/usr/bin/env python3
"""Insert or update one Blockchain & Tech item in Tech Notes from two markdown files.

    python3 scripts/add-tech-item.py --key <key> --slot <N> --en <en.md> --ko <ko.md> \
        [--status new|important|planned|recent|done] [--date YYYY-MM-DD] [--type PoC]
...
sec = next(x for x in nav["sections"] if x["navId"] == "nav-sec-blockchain")
items = sec["items"]
...
items.insert(slot - 1, {"key": KEY, "href": HREF, "color": COLOR, "label": LABEL, "text": E(TITLE)})
...
sec["label"] = f"Blockchain & Tech ({N})"
for j in nav["jump"]:
    if j["id"] == "sec-blockchain": j["all"], j["done"] = N, DONE
...
items = [[x["key"], x["href"], x["color"], x["label"], str(k), re.sub(r'^<span class="topic-no">\d+</span>', '', x["text"])] for k, x in enumerate(items, 1)]

# ---------------- notes.html ----------------
p = ROOT / "notes.html"; s = p.read_text()
a0 = s.index('id="nav-sec-blockchain"'); a1 = s.index('id="nav-sec-fundamentals"')
b0 = s.index('<article id="sec-blockchain">'); b1 = s.index('<article id="sec-fundamentals">')
...
navli = (f'        <li><a class="nav-link" href="#{KEY}" data-key="{KEY}"><span class="nav-dot" style="background:{COLOR};" title="{LABEL}"></span>'
         f'<span class="nav-text"><span class="topic-no">{slot}</span>{E(TITLE)}</span></a></li>\n')
card = f'''        <li id="{KEY}">
          <div class="topic-head"><span class="topic-no">{slot}</span><span class="topic-title">{E(TITLE)}</span>{DATE_SPAN}</div>
          ...
          <p class="topic-link"><a href="topics/{HREF}">Detail &rarr;</a> &middot; <a href="#top">Top &uarr;</a> &middot; <a href="#sec-blockchain">Section top &uarr;</a> &middot; <button type="button" class="copy-btn copy-inline" data-copy="copy-en-{KEY}" data-done="Copied &#10003;">Copy EN</button><button type="button" class="copy-btn copy-inline" data-copy="copy-ko-{KEY}" data-done="복사됨 &#10003;">Copy KO</button><script type="application/json" id="copy-en-{KEY}">{json.dumps(en_md, ensure_ascii=False)}</script><script type="application/json" id="copy-ko-{KEY}">{json.dumps(ko_md, ensure_ascii=False)}</script></p>
        </li>
'''
if slot == 1:
...
s = re.sub(r'Blockchain & Tech &mdash; done \(\d+\) / all \(\d+\)', f'Blockchain & Tech &mdash; done ({DONE}) / all ({N})', s, count=1)
s = re.sub(r'(<a href="#sec-blockchain"[^>]*>Blockchain & Tech<b><span class="count-done">)\d+(</span><span class="count-all">/)\d+', lambda m: f'{m.group(1)}{DONE}{m.group(2)}{N}', s, count=1)
s = re.sub(r'(<article id="sec-blockchain">.*?<span class="count-done">done \()\d+(\)</span> <span class="count-all">/ all \()\d+', lambda m: f'{m.group(1)}{DONE}{m.group(2)}{N}', s, count=1, flags=re.S)
...
mid = f'''    <p class="crumb"><a href="../index.html">Workspace Index</a> &rsaquo; <a href="../notes.html">Tech Notes</a> &rsaquo; {E(TITLE)}</p>
  <header class="topic-hero">
      <p class="topic-kicker"><span class="topic-no">#{slot}</span><span>{E(A.type)}</span><span title="added">{DATE}</span></p>
      <h1>{E(TITLE)}</h1>
...
print(f"#{slot} {KEY} [{LABEL}, {DATE}] → Blockchain {DONE}/{N}, overall {badge}; pages rewritten {fixed}, missing files {missing}")
