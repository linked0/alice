...
PAGE_JS = r'''<script>
(function () {
  // Locked Life item (jay, 2026-09-18; moved to Firestore 2026-09-21). The ciphertext is no longer in
  // this repo — it lives in the `health/bundle` document and is fetched after Google sign-in. The key
  // is still derived here from the password, so the database holds bytes nobody but jay can read:
  // sign-in decides who may FETCH, the password decides who may READ. Rules do not protect against a
  // service account, and this project has ones with project-wide access, so both layers are kept.
  ...
  var CFG = { apiKey: 'AIzaSyDS_cerGsvcrannER-irNx2Y3kGRQ3ybbk', authDomain: 'doubletree-498007.firebaseapp.com', projectId: 'doubletree-498007', appId: '1:179807446244:web:bfa1f0500d173c599538f1' };
  ...
  function showGate() { slot.innerHTML = ''; slot.appendChild(gate); status.innerHTML = '&#128274; Locked. Sign in, then enter the password.'; input.focus(); }
  ...
  // Fetch the encrypted bundle once per tab. Signing in is part of fetching, not a separate step the
  // reader has to think about; a failure clears the cache so the next attempt retries cleanly.
  var blobP = null;
  function getBlob() {
    if (blobP) return blobP;
    blobP = Promise.all([import(SDK + 'firebase-app.js'), import(SDK + 'firebase-auth.js'), import(SDK + 'firebase-firestore.js')]).then(function (m) {
      var app = m[0].initializeApp(CFG), auth = m[1].getAuth(app), db = m[2].getFirestore(app);
      function read() {
        return m[2].getDoc(m[2].doc(db, 'health', 'bundle')).then(function (s) {
          if (!s.exists()) throw new Error('no bundle');
          return s.data();
        });
      }
      return new Promise(function (res) { var un = m[1].onAuthStateChanged(auth, function (u) { un(); res(u); }); })
        .then(function (u) { return u ? read() : m[1].signInWithPopup(auth, new m[1].GoogleAuthProvider()).then(read); });
    });
  ...
  async function unlock(pass) {
    ...
    var key = await crypto.subtle.deriveKey({ name: 'PBKDF2', salt: b64(B.salt), iterations: B.iter, hash: 'SHA-256' }, base, { name: 'AES-GCM', length: 256 }, false, ['decrypt']);
    return JSON.parse(new TextDecoder().decode(await crypto.subtle.decrypt({ name: 'AES-GCM', iv: b64(B.iv) }, key, b64(B.ct))));
  }
  async function tryUnlock(pass, remember) {
    msg.textContent = 'Fetching…'; go.disabled = true;
    try {
      ...
      msg.textContent = /auth|permission|network|bundle/i.test(code) ? code : 'Wrong password.';
      if (!gate.parentNode) showGate();
    }
    go.disabled = false;
  }
  ...
  slot.addEventListener('click', function (e) { if (e.target.closest('.health-lockall')) { try { sessionStorage.removeItem('life-health-pass'); } catch (x) {} blobP = null; showGate(); } });
  showGate();
