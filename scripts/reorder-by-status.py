...
SECTIONS = {"nav-sec-blockchain": "sec-blockchain", "nav-sec-fundamentals": "sec-fundamentals"}
NEXT = {"nav-sec-blockchain": ("nav-sec-fundamentals", "sec-fundamentals"), "nav-sec-fundamentals": ("nav-sec-mindset", "sec-mindset")}
...
for navid, artid in SECTIONS.items():
    ...
    sec["items"].sort(key=lambda x: RANK[x["label"]])          # stable
    ...
    num = {key: k for k, key in enumerate(order, 1)}
    ...
    newcards = "".join(re.sub(r'(<div class="topic-head"><span class="topic-no">)\d+', lambda m, k=k: m.group(1) + str(num[k]), cs[k], count=1) for k in order)
    s = s[:b0] + cards[:first] + newcards + cards[last:] + s[b1:]
    ...
    for idx, x in enumerate(items):
        ...
        c2 = re.sub(r'<span class="topic-no">#\d+</span>', f'<span class="topic-no">#{idx + 1}</span>', c, count=1)
        prev = items[idx - 1] if idx > 0 else None; nxt = items[idx + 1] if idx + 1 < len(items) else None
        pager = '<div class="pager">' + (f'<a href="{prev["href"]}">&larr; {idx}. {title(prev)}</a>' if prev else '') + (f'<a href="{nxt["href"]}">{idx + 2}. {title(nxt)} &rarr;</a>' if nxt else '') + '</div>'
        c2 = re.sub(r'<div class="pager">.*?</div>', lambda _: pager, c2, count=1, flags=re.S)
        if c2 != c: f.write_text(c2)
    print(f"{navid}: {moved} items changed number; order now " + "".join({0: "D", 1: "I", 2: "N", 3: "P"}[RANK[x["label"]]] for x in items))
