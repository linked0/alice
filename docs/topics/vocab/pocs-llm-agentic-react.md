| Expression | 뜻 · 쓰이는 자리 |
|---|---|
| reason-act-observe loop | 추론-행동-관찰을 반복하는 루프(ReAct 방식) · 에이전트가 여러 단계에 걸쳐 계획하고 도구를 쓰는 구조 · "The reason-act-observe loop lets a model plan" |
| run away in cost | 비용이 걷잡을 수 없이 불어나다 · 에이전트가 무한정 반복하며 비용이 폭주하는 위험 · "or run away in cost" |
