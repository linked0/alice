...
| rate card | 요금표 · 서비스별 가격을 정리한 공식 가격표. "written into a rate card" |
| byte-stable | 바이트 단위로 전혀 바뀌지 않는 · 캐시 적중을 위해 프롬프트가 한 글자도 안 바뀌어야 함을 가리킬 때. "prompt prefixes must be byte-stable" |
